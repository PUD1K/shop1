    ...
    <div class="page-checkout">
        <div class="columns is-multiline">
            ...
            <div class="column is-12 box">
                ...
                <p class="has-text-grey mb-4">* All fields are requried</p>
...
export default {
    ...
    mounted(){
        document.title = "Checkout # Thing"

        this.cart = this.$store.state.cart

        // если товары есть в корзине
        if(this.cartTotalLength > 0){
            this.stripe = Stripe('pk_test_51KmJUOIwSOHgTMzsno9bK0o1Bh7MEZoLy7TSKM1EHuk7PV22NPysSDwFodnOt4buqU1DtG0KMoyukQLuzXrKghUW003YlOuPLW')
            const elements = this.stripe.elements();
            // скрываем почтовый индекс
            this.card = elements.create('card', {hidePostalCode: true })
            // хук - mount  
            // отправляется http запрос на получение данных для передачи их в DOM(для id - card-element) для последующего рендера
        ...
        submitForm(){
            ...
            if(!this.errors.length){
                ...
                this.stripe.createToken(this.card).then(result => {
                    ...
                    // если ошибок нет, вызываем токен обработчика stripe
                    else{
